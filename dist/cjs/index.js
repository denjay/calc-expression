...
const numberRegString = String.raw `(?:Infinity|NaN|${scientificRegString}|${floatRegString})`;
 ...
 */
function getExpressionWithoutParentheses(expression) {
    ...
    bracketPairArr.forEach((bracketPair, index) => {
        if (index === 0) {
            ...
            newExpression += calc(expression.slice(bracketPair[0] + 1, bracketPair[1]));
        }
        else {
            newExpression += expression.slice(bracketPairArr[index - 1][1] + 1, bracketPair[0]);
            newExpression += calc(expression.slice(bracketPair[0] + 1, bracketPair[1]));
        }
 ...
 */
function getBracketPairArr(expression) {
    const bracketPairArr = []; // 记录第一层的括号对信息
    const leftBracketIndexArr = []; // 记录左括号的位置
 ...
 */
function calculateExpressionWithoutParentheses(expression) {
    ...
    operatorArrList.forEach((operatorArr) => {
        ...
        while (!expression.match(String.raw `^\s*[\+\-]?${minUnitRegString}\s*$`) &&
            operatorArr.some((operator) => expression.match(String.raw `${minUnitRegString}\s*${operator.replace(/(.)/g, "\\$1")}\s*${minUnitRegString}`))) {
            const regResult = expression.match(String.raw `${minUnitRegString}\s*(${operatorArr
                .map((operator) => operator.replace(/(.)/g, "\\$1"))
